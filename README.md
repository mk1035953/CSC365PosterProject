...
This is the repository for the poster project. The cv2 image library for Python was used to take low sample path-traced images and turn them into clean images. The Cycles engine is required to make path-traced images as Blender puts it, “Cycles is Blender’s physically-based path tracer for production rendering. It is designed to provide physically based results out-of-the-box, with artistic control and flexible shading nodes for production needs.” (Blender 2019) But to make really good images or animations requires a lot of computation power, so this project is designed to reduce render costs and still make good quality images.
...
Download one of the three models below in your choice of format (if you would like to use a different model then feel free to import it on your local machine)
